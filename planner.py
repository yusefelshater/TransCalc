...
DEFAULT_WEIGHTS = {
    ...
    "ready_mix_proximity": 1.0,        # proximity to ready-mix concrete plants
    "bitumen_source_proximity": 1.0,   # proximity to bitumen sources (if present in OSM)
}

# Search radii (meters)
SEARCH_RADIUS_M = 50000  # 50 km around path bbox
...
def path_midpoint(path: List[Tuple[float, float]]) -> Tuple[float, float]:
    # Simple midpoint by index; not length-weighted
...
# -------------------- Overpass Queries --------------------
def overpass_post(query: str, timeout_s: int = 45, retries: int = 2) -> Dict[str, Any]:
    ...
    # simple backoff schedule in seconds
    backoff = [1, 3, 6]
...
def score_candidate(point: Tuple[float, float], path: List[Tuple[float, float]],
    ...
    base_score = (
        ...
        weights.get("ready_mix_proximity", 1.0) * ready_mix_score +
...
def analyze_path(path: List[Tuple[float, float]], mode: str = "new", top_k: int = 5,
    ...
    # Load fallback facilities (local JSON) and keep only those within 50 km of the path
    ...
    fallback_asphalt = _annotate_and_filter(fb.get("asphalt_plants", []), "fallback_asphalt")
    fallback_waste = _annotate_and_filter(fb.get("waste_sites", []), "fallback_waste")
    fallback_rubber_recycling = _annotate_and_filter(fb.get("rubber_recycling", []), "fallback_rubber_recycling")
    fallback_rubber_production = _annotate_and_filter(fb.get("rubber_production", []), "fallback_rubber_production")
    ...
    # Propose new sites: midpoint + quarter points along the path
    idxs = [len(path)//4, len(path)//2, (3*len(path))//4]
    proposed_points = []
    used = set()
    for i in idxs:
    ...
    proposed_top = proposed_scored[:top_k]
