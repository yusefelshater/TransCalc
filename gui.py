...
class PavementApp(ctk.CTk):
    def __init__(self):
        ...
        # Sidebar widgets
        self.logo_label = ctk.CTkLabel(self.sidebar, text="Road-Calc", font=ctk.CTkFont(size=20, weight="bold"))
    ...
    def is_intro_enabled(self) -> bool:
        """Read INTRO_ENABLED env (default True)."""
        try:
            v = os.environ.get("INTRO_ENABLED")
            if v is None:
                return True
    ...
    def get_intro_timeout_seconds(self) -> float:
        """Read INTRO_TIMEOUT_S env; default to get_intro_duration_seconds()."""
        try:
            v = os.environ.get("INTRO_TIMEOUT_S")
            if v is not None:
                return max(1.0, float(v))
        except Exception:
            pass
        return float(self.get_intro_duration_seconds())
    ...
    def play_intro_then_show(self):
        ...
        timeout_s = self.get_intro_timeout_seconds()
        if video:
            try:
                if ffplay_path:
                    ...
                    try:
                        res = subprocess.run(args, check=False, timeout=timeout_s)
                    except subprocess.TimeoutExpired:
                        # Timed out: proceed to show app
                        self.safe_show()
                        return
                    if res.returncode != 0:
                        ...
                        try:
                            subprocess.run(fallback_args, check=False, timeout=timeout_s)
                        except subprocess.TimeoutExpired:
                            self.safe_show()
                            return
                    self.safe_show()
                ...
                if vlc_path:
                    ...
                    try:
                        subprocess.run(vlc_args, check=False, timeout=timeout_s)
                    except subprocess.TimeoutExpired:
                        self.safe_show()
                        return
                    self.safe_show()
                ...
                if selected:
                    ...
                    if base == "ffplay.exe":
                        ...
                        try:
                            subprocess.run(args, check=False, timeout=timeout_s)
                        except subprocess.TimeoutExpired:
                            self.safe_show()
                            return
                        self.safe_show()
    ...
    def create_inputs(self, tab):
        ...
        self.params = {
            ...
            "W": {"label": "Road Width (m)", "default": "7.0"},
            "h": {"label": "Layer Thickness (m)", "default": "0.05"},
            "rho_m": {"label": "Mixture Density (ton/m³)", "default": "2.35"},
            "Pb": {"label": "Bitumen Content (proportion)", "default": "0.055"},
            "Pp": {"label": "Plastic Content (proportion of bitumen)", "default": "0.05"},
            "Pr": {"label": "Rubber Content (proportion of bitumen)", "default": "0.02"},
            "T": {"label": "Temperature (°C)", "default": "30.0"},
            "A": {"label": "Annual ESALs (millions)", "default": "1.0"},
            "c_agg": {"label": "Aggregate Cost (per ton)", "default": "165.0"},
            "c_bit": {"label": "Bitumen Cost (per ton)", "default": "66925.0"},
            "c_pl": {"label": "Plastic Cost (per ton)", "default": "200.0"},
            "c_rub": {"label": "Rubber Cost (per ton)", "default": "12500.0"},
            "overhead": {"label": "Overhead Cost (managed in Overheads)", "default": "0.0"},
            "target_design_life": {"label": "Target Design Life (years)", "default": "15"}
        }

        # Initialize unit-cost defaults from catalog when available
        try:
            catalog = self.catalog if isinstance(self.catalog, dict) else {}
            # Bitumen and rubber from catalog
            v_bit = (catalog.get("bitumen") or {}).get("price_per_ton") if isinstance(catalog, dict) else None
            if v_bit is not None and "c_bit" in self.params:
                self.params["c_bit"]["default"] = str(v_bit)
            v_rub = (catalog.get("rubber") or {}).get("price_per_ton") if isinstance(catalog, dict) else None
            if v_rub is not None and "c_rub" in self.params:
                self.params["c_rub"]["default"] = str(v_rub)
            # Aggregates blended price per ton from baseline shares/types
            ...
            cat_items = {it.get("id"): it for it in ((catalog.get("aggregates_catalog") or []) if isinstance(catalog, dict) else []) if isinstance(it, dict)}
            shares = []
            prices = []
            for cat in ("coarse", "medium", "fine"):
                cfg = (agg_cfg.get(cat) or {}) if isinstance(agg_cfg, dict) else {}
                frac = float(cfg.get("fraction_of_mix", 0.0) or 0.0)
                tid = cfg.get("type_id")
                price = float(((cat_items.get(tid) or {}).get("price_per_ton")) or 0.0)
                if frac > 0.0 and price > 0.0:
                    shares.append(frac)
                    prices.append(price)
            if shares and "c_agg" in self.params:
                s = sum(shares)
                if s > 0:
                    avg = sum((shares[i] / s) * prices[i] for i in range(len(shares)))
                    self.params["c_agg"]["default"] = f"{avg:.6g}"
        except Exception:
            pass

        # Remove legacy overhead field entirely from Inputs; it will be computed from the Overheads tab
        try:
            self.params.pop("overhead", None)
        except Exception:
            pass

        # If plastic is disabled by catalog flag, remove its fields from the UI
        if not self.is_plastic_enabled():
            try:
                self.params.pop("Pp", None)
                self.params.pop("c_pl", None)
            except Exception:
                pass
        ...
        # Initialize auto-managed unit costs snapshot (used to detect user overrides)
        try:
            self.auto_unit_costs = {}
            for k in ("c_agg", "c_bit", "c_rub", "c_pl"):
                if k in self.entries:
                    try:
                        self.auto_unit_costs[k] = self.entries[k].get().strip()
                    except Exception:
                        self.auto_unit_costs[k] = None
        except Exception:
            self.auto_unit_costs = {}
    ...
    def is_plastic_enabled(self) -> bool:
        """Determine if plastic feature is enabled from catalog (default True)."""
        try:
            catalog = self.catalog if isinstance(self.catalog, dict) else {}
            pl = catalog.get("plastic") if isinstance(catalog, dict) else None
    ...
    def build_mix_inputs_from_gui(self) -> dict:
        ...
        # Optional unit-cost overrides from GUI entries
        unit_cost_overrides = {}
        try:
            if "c_bit" in self.entries:
                v = float(self.entries.get("c_bit").get())
                if v > 0:
                    unit_cost_overrides["bitumen_price_per_ton"] = v
        except Exception:
            pass
        try:
            if "c_rub" in self.entries:
                v = float(self.entries.get("c_rub").get())
                if v > 0:
                    unit_cost_overrides["rubber_price_per_ton"] = v
        except Exception:
            pass
        ...
        # Attach overrides only if present
        if unit_cost_overrides:
            mix_inputs["unit_cost_overrides"] = unit_cost_overrides
        return mix_inputs
    ...
    def run_model(self):
        ...
        # Ensure plastic inputs are zeroed when feature is disabled
        if not self.is_plastic_enabled():
            input_values["Pp"] = 0.0
            input_values["c_pl"] = 0.0
        ...
        if isinstance(Pb, float) and not (0.04 <= Pb <= 0.07):
            gui_warnings.append(
                f"⚠ Bitumen content = {Pb:.3f} (خارج النطاق 0.04–0.07). تم إجراء الحسابات على القيمة المُدخلة، وقد تكون النتائج غير واقعية."
            )
            try:
        ...
        if isinstance(Pr, float) and not (0.01 <= Pr <= 0.60):
            gui_warnings.append(
                f"⚠ Rubber content = {Pr:.3f} (من البيتومين) خارج النطاق 0.01–0.60. تم إجراء الحسابات على القيمة المُدخلة، وقد تكون النتائج غير واقعية."
            )
            try:
        ...
        try:
            ...
            # Dynamically override unit prices from catalog/mix and sync GUI fields
            try:
                catalog = self.catalog or {}
                # Aggregates blended price per ton from mix breakdown
                agg_mass = float(((mix_res.get("quantities") or {}).get("aggregates_total_ton", 0.0)) or 0.0)
                agg_sub = float(((mix_res.get("costs") or {}).get("aggregates_subtotal", 0.0)) or 0.0)
                c_agg_dyn = (agg_sub / agg_mass) if agg_mass > 1e-12 else 0.0
                def _maybe_override_cost(key: str, new_val: float):
                    try:
                        if new_val <= 0.0:
                            return False
                        ...
                        if not e:
                            return False
                        curr = e.get().strip()
                        prev_auto = (self.auto_unit_costs or {}).get(key)
                        def _as_float(s):
                            try:
                                return float(s)
                            except Exception:
                                return None
                        currf = _as_float(curr)
                        prevf = _as_float(prev_auto)
                        # If entry still equals last auto value, we can override; otherwise keep user's value
                        if prevf is not None and currf is not None and abs(currf - prevf) < 1e-9:
                            input_values[key] = new_val
                            try:
                                e.delete(0, "end"); e.insert(0, f"{new_val:.6g}")
                            except Exception:
                                pass
                            try:
                                self.auto_unit_costs[key] = f"{new_val:.6g}"
                            except Exception:
                                pass
                            return True
                        return False
                ...
                if c_agg_dyn > 0.0:
                    _maybe_override_cost("c_agg", c_agg_dyn)
                # Bitumen and rubber prices from catalog
                c_bit_dyn = float(((catalog.get("bitumen") or {}).get("price_per_ton")) or 0.0)
                if c_bit_dyn > 0.0:
                    _maybe_override_cost("c_bit", c_bit_dyn)
                c_rub_dyn = float(((catalog.get("rubber") or {}).get("price_per_ton")) or 0.0)
                if c_rub_dyn > 0.0:
                    _maybe_override_cost("c_rub", c_rub_dyn)
                # Plastic price only if feature enabled and catalog provides it
                if self.is_plastic_enabled():
                    c_pl_dyn = float(((catalog.get("plastic") or {}).get("price_per_ton")) or 0.0)
                    if c_pl_dyn > 0.0:
                        _maybe_override_cost("c_pl", c_pl_dyn)
            except Exception:
                pass
        except Exception as e_mix:
            # If catalog costing fails, do NOT fall back to legacy field. Force overhead to 0 and warn.
            try:
                messagebox.showwarning("Catalog Costing", f"تعذّر حساب كلفة الكتالوج: {e_mix}\nسيتم اعتبار المصاريف العامة = 0 مؤقتًا. من فضلك راجع تبويب Overheads ثم أعد التشغيل.")
            except Exception:
                pass
            input_values["overhead"] = 0.0
    ...
    def display_results(self, results):
        ...
        for key, value in results.items():
            if key != "warnings":
                if key == "costs" and not self.is_plastic_enabled() and isinstance(value, dict):
                    # Hide plastic component from display when disabled
                    filtered = {k: v for k, v in value.items() if k != "plastic"}
                    result_str += f"{key}: {filtered}\n"
                elif key == "mix_results" and not self.is_plastic_enabled() and isinstance(value, dict):
                    # Hide plastic subtotal from nested catalog costs when disabled
                    try:
                        vcopy = dict(value)
                        costs = dict((value.get("costs") or {}))
                        if "plastic_subtotal" in costs:
                            costs.pop("plastic_subtotal", None)
                        vcopy["costs"] = costs
                        result_str += f"{key}: {vcopy}\n"
                    except Exception:
    ...
    def update_plots(self, results: dict):
        ...
        mix_res = results.get("mix_results") if isinstance(results.get("mix_results"), dict) else None
        # Omit plastic from charts if disabled
        if self.is_plastic_enabled():
            cost_labels = ["aggregate", "bitumen", "plastic", "rubber", "overhead"]
        else:
            cost_labels = ["aggregate", "bitumen", "rubber", "overhead"]
        if isinstance(mix_res, dict):
            ...
            # plastic is intentionally omitted when disabled; mapping has no key then
            if self.is_plastic_enabled():
                mapping["plastic"] = float(mc.get("plastic_subtotal", 0.0))
            cost_values = [mapping.get(k, 0.0) for k in cost_labels]
        else:
        ...
        try:
            ...
            if seg_len_km > 0 and bidir:
                ...
                def _fmt_fb_list(items, label):
                    if not items:
                        return [f"  لا يوجد {label} ضمن 200 كم"]
                ...
                def _fmt_fb_list(items, label):
                    ...
                    return out
                lines.append("")
                lines.append("Fallback facilities within 200 km:")
                lines.extend(_fmt_fb_list(res.get('fallback_asphalt', []), "محطات أسفلت (احتياط)"))
                lines.extend(_fmt_fb_list(res.get('fallback_waste', []), "مواقع مخلفات/نفايات (احتياط)"))
                lines.extend(_fmt_fb_list(res.get('fallback_rubber_recycling', []), "مصانع تدوير مطاط (احتياط)"))
                lines.extend(_fmt_fb_list(res.get('fallback_rubber_production', []), "مصانع إنتاج مطاط (احتياط)"))
