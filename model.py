...
def calculate_mix(inputs: dict, catalog: dict) -> dict:
    ...
    if L_km < 0 or W_m < 0 or h_m < 0 or rho <= 0:
        raise ValueError("Invalid project inputs (negative or zero density)")
    ...
    if isinstance(br, list) and len(br) == 2 and not (br[0] <= Pb <= br[1]):
        warnings.append(f"تحذير: نسبة البيتومين {Pb:.3f} خارج النطاق الموصى به [{br[0]:.3f}–{br[1]:.3f}].")
    if isinstance(rr, list) and len(rr) == 2 and not (rr[0] <= Rb <= rr[1]):
        warnings.append(f"تحذير: نسبة المطاط من البيتومين {Rb:.3f} خارج النطاق [{rr[0]:.3f}–{rr[1]:.3f}].")
    ...
    if aggregates_total_ton < -1e-9:
        raise ValueError("Aggregates total mass became negative — check fractions.")
    ...
    # Binder costs
    bitumen_price = float(((catalog.get("bitumen") or {}).get("price_per_ton")) or 0.0) if isinstance(catalog, dict) else 0.0
    rubber_price = float(((catalog.get("rubber") or {}).get("price_per_ton")) or 0.0) if isinstance(catalog, dict) else 0.0
